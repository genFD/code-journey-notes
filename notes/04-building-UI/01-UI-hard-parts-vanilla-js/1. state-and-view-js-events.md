...
4. the output of calling querySelector is `an accessor object full of properties and methods that allow us to interact with the selected dom element` : `const jsdiv = document.querySelector('div')`
...
1. html code is parsed

2. three nodes are added to the DOM

3. Adding the `script node` triggers JS engine

4. Defined a variable `post` and assign to it the string '`hi`'

5. Defined the constant `jsdiv` and assign it the result of calling the querySelector method which is on the document object passing to it the string '`div`'

- `querySelector` method is going to query(search) the first element in the list of type div

- Normally we would bring back that element in JS, but this is impossible because it's two completely different runtime. We cannot bring back in JS something that was written in C++. What really happens is querySelector is going to create an object in JS that's going to have an hidden link the div element in C++. This object is also going to be populated with a lot of functions and properties (like textConent) to interact with that div.

6. We accessed jsdiv object and set the textContent property of the div dom element to be the value of post.

This is how we use JS and the DOM to create and save data/state and display it on the screen.
...
`TLDR` : Thanks to `DOM events` and `event handlers`.
Events are actions that a user take. Event handlers are functions that are triggered as a result of users actions.
...
4. defined the variable `post` and assign to it an empty string

5. defined the constant jsInput and assigned to it the result of calling the `querySelector` method passing to it the string input. As a result, an accesor object is created with an hidden link to the input element on the DOM. This object is also full of getters and setters methods that allows us to interact with this DOM element such as `value`, `onInput`.

6. defined the constant jsDiv and assigned to it the result of calling the querySelector method passing in the string div. As a result, an accesor object is created with an hidden link to the first div element on the DOM. This object is also full of getters and setters methods that allows us to interact with the DOM element such as , `textContent`

7. defined a function called `handleInput`

8. Accessed theobject jsInput that has link to our `<input>` node in C++, set the setter property `onInput` to be the function handleInput. As a result `handleInput` is set (technically a reference) to be the handler of the input element. This means that anytime the user types something in the input that function will be call back into js and executed.

9. When the user type '`hi`' in the input element, the value prop of input element is set to '`hi`'.

10. The function `handleInput` is automatically triggered. The function goes into the cb queue and then is pushed onto the call stack. A brand new execution context is created :

- `post` is reassigned the value entered by the user
- the div textContent is reassigned to `post`

11. As a result the DOM is updated with the new values from JS and the page re-renders.
