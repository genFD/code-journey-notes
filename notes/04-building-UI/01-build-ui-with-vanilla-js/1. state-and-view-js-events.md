...
1. HTML allows us to list the elements that we want to display on the page (text, box, input, etc..)
2. We load it into the web browser
3. HTML code is parsed(read) by the HTML parser and turn into a list (written in C++ - web browser language) which is simplified version or representation of the page.
4. Finally, the render engine will transform that list into a page that a user can see.
...
2. That list of elements in HTML produces a list of elements in c++ called the DOM - Model (representation) of the page. We can imagine that this list would look something like the following:
...
3. That List of elements in c++ is then converted into actual pixels rendered on the page
...
So to enable users to change the content, we need to enable them to change the underlying data.
...
- In our `js` file :
  ...
  - We defined a constant `jsInput` and assigned to it the return value of calling the querySelector method passing in string `input`. The ouput is an accesor _object_ with an hidden link to the `input` element on the DOM. This object is also full of methods and properties (such as `value`, `onInput`) that allows us to interact with this DOM node .

  - On the next line, we defined the constant `jsDiv` and assigned to it the return value of calling the querySelector method passing in the string `div`. The ouput is an accesor _object_ with an hidden link to to the first div element on the DOM. This object is also full of methods and properties (such as `textContent`) that allows us to interact with this DOM node.

  - Next, we defined a function called `handleInput`

  - Finally, we access the object `jsInput` and we set the property `onInput` to be the function handleInput. (The technical term for handleInput is an event handler). This means that whenever a user enters or deletes any character inside the `<input>` that handleInput will be call back and executed.

For example, if a user type '`hi`' in the input element :

- The value property of the input element will be set to '`hi`'.

- The function `handleInput` will be automatically triggered. The function goes into the callback queue and then is pushed onto the call stack. A brand new execution context is created :
...
Every change of data (like a user input) is going to require a back and forth between C++ (DOM) and JavaScript (app.js). There's no permanent bond in JS memory and in C++ memory.
